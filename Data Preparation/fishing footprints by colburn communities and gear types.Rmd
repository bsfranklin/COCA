...
vtr.data <- readRDS("I:/jschuetz/Documents/vtr.data.rds") %>%
  group_by(JGS.COMMUNITY, COST_ID) %>%
  nest()
...
MakeNOAASafeCommunityGearFootprints <- function(df, list.column) {
  ...
  raster.kept.safe <- list.column[[2]] * footprint.safe
  raster.proportion.safe <- list.column[[2]] * footprint.safe
  
  list(footprint.safe, raster.kept.safe, raster.proportion.safe)
...
shelf.kept <- stack()
shelf.proportion <- stack()

for (g in 1:length(gear.type)){
  
  gear.stack <- kept.stack[[seq(gear.type[g], 7 * nrow(community.name), length(gear.type))]]
  gear.total <- sum(gear.stack, na.rm = T)
  names(gear.total) <- str_c("SHELFWIDE_GEAR_TYPE_", gear.type[g])
  shelf.kept <- stack(shelf.kept, gear.total)
  
  layer.total <- cellStats(gear.total, stat = "sum", na.rm = T)
  gear.proportion <- gear.total/layer.total
  names(gear.proportion) <- str_c("SHELFWIDE_GEAR_TYPE_", gear.type[g])
  shelf.proportion <- stack(shelf.proportion, gear.proportion)
